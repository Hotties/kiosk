    ...
    try:
        ...
        try:
            with conn.cursor() as cursor:
                ...
                # 3. SIDEMENU 테이블
                if SheetName.SIDE_MENU.name in excel_data:
                    print(f"📝 '{SheetName.SIDE_MENU.name}' 시트 데이터 로드 중...")
                    handler = create_sheet_handler(SheetName.SIDE_MENU.name, cursor)
                    sidemenu_rows = excel_data[SheetName.SIDE_MENU.name]
                    
                    print("side_menu_rows:", sidemenu_rows)  # 디버깅용 출력

                    for row in sidemenu_rows:
                        if row.get('menu_name') and row.get('price') is not None:
                            handler.insert_row(row)
